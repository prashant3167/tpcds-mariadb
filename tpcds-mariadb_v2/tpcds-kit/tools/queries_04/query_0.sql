...
select  c_last_name
 ...
 from
   (select ss_ticket_number
    ...
    and store.s_city in ('Fairview','Oak Grove','Midway','Midway','Fairview') 
...
select  
 ...
 where
  ...
  ca_county in ('Halifax County','Scott County','Monongalia County','Harmon County','Montgomery County') and
...
from (select avg(ss_list_price) B1_LP
      ...
      where ss_quantity between 0 and 5
        and (ss_list_price between 58 and 58+10 
             or ss_coupon_amt between 12572 and 12572+1000
     ...
     (select avg(ss_list_price) B3_LP
      ...
      where ss_quantity between 11 and 15
        and (ss_list_price between 122 and 122+10
          or ss_coupon_amt between 5808 and 5808+1000
...
select   
 ...
 from (
 	...
 	and ws_ship_mode_sk = sm_ship_mode_sk
        and d_year = 2000
 	and t_time between 19142 and 19142+28800 
...
select case when (select count(*) 
                  from store_sales 
                  where ss_quantity between 1 and 20) > 144588
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 21 and 40) > 185849
            then (select avg(ss_ext_tax)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 41 and 60) > 49194
            then (select avg(ss_ext_tax)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 61 and 80) > 943
            then (select avg(ss_ext_tax)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 81 and 100) > 119200
            then (select avg(ss_ext_tax)
                  from store_sales
...
where ss_item_sk = i_item_sk and
      ...
      d_year in (1999) and
        ((i_category in ('Shoes','Men','Books') and
          i_class in ('womens','accessories','science')
         )
      or (i_category in ('Music','Sports','Jewelry') and
          i_class in ('pop','outdoor','pendants') 
        ))
...
select  c_customer_id as customer_id
 ...
 where ca_city	        =  'Jackson'
   and c_current_addr_sk = ca_address_sk
   and ib_lower_bound   >=  54493
   and ib_upper_bound   <=  54493 + 50000
...
with ss as (
 ...
 where i_item_id in (select
     i_item_id
from item
where i_color in ('purple','tan','slate'))
 and     ss_item_sk              = i_item_sk
 and     ss_sold_date_sk         = d_date_sk
 and     d_year                  = 2000
 and     d_moy                   = 3
...
select  i_item_id,
 ...
 where cs_sold_date_sk = d_date_sk and
       ...
       c_birth_month in (1,5,3,11,12,8) and
       d_year = 2001 and
...
select avg(ss_quantity)
 ...
 and((ss_addr_sk = ca_address_sk
  ...
  and ca_state in ('TX', 'KY', 'MS')
  and ss_net_profit between 100 and 200  
     ) or
     (ss_addr_sk = ca_address_sk
  and ca_country = 'United States'
  and ca_state in ('ID', 'KS', 'MT')
  and ss_net_profit between 150 and 300  
     ) or
     (ss_addr_sk = ca_address_sk
  and ca_country = 'United States'
  and ca_state in ('ND', 'NE', 'MO')
...
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in ('1999-03-03','1999-08-03','1999-11-09')))
